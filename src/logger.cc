...
#define LOGGER_ANSI_RESET  "\033[0m"
#define LOGGER_ANSI_RED    "\033[0;31m"
#define LOGGER_ANSI_YELLOW "\033[0;33m"
#define LOGGER_ANSI_GREEN  "\033[0;32m"
#define LOGGER_ANSI_BLUE   "\033[0;34m"
#define LOGGER_ANSI_PURPLE "\033[0;35m"
...
static void LoggerTime(std::ostream &out_stream)
{
    ...
    // Display the local time in HH:MM:SS.mmm format
    out_string_stream << std::format("{:%H:%M:%S}", std::chrono::zoned_time{std::chrono::current_zone(), std::chrono::floor<std::chrono::seconds>(now)})
                      << '.' << std::setfill('0') << std::setw(LOGGER_MS_WIDTH) << ms.count();
